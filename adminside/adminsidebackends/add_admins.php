...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $checkEmailSql = "SELECT 1 FROM admins WHERE email = ?";
    ...
    if ($resultEmail->num_rows > 0) {
        echo json_encode(['success' => false, 'message' => 'A user with the same email already exists']);
        exit();
    }

    // Insert data into the admins table
    $sql = "INSERT INTO admins (fname, mname, lname, ename, nickname, email, age, sex, birthdate, address, username, password) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssssss", $fname, $mname, $lname, $ename, $nickname, $email, $age, $sex, $birthdate, $address, $username, $password);
