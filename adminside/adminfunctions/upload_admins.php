...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    ...
    if ($xlsx = SimpleXLSX::parse($file)) {
        ...
        foreach ($rows as $index => $row) {
            ...
            // Check if a user with the same username, nickname, or full name (including ename) already exists
            $checkSql = "SELECT * FROM admins WHERE username = ? OR nickname = ? OR (fname = ? AND mname = ? AND lname = ? AND ename = ?)";
            $checkStmt = $conn->prepare($checkSql);
            $checkStmt->bind_param("ssssss", $username, $nickname, $fname, $mname, $lname, $ename);
            $checkStmt->execute();
            $checkResult = $checkStmt->get_result();

            if ($checkResult->num_rows > 0) {
                $errors[] = "Row $index: A user with the same username, nickname, or full name already exists.";
                $checkStmt->close();
                continue;
            }
            $checkStmt->close();
            ...
            $sql = "INSERT INTO admins (fname, mname, lname, ename, nickname, age, sex, birthdate, address, username, password, email) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
